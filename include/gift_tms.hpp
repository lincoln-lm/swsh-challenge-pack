...
constexpr TMFunctionIdentifier TM_IDENTIFIERS[] = {
    ...
    { "melony_icy_wind", 0xa2e8, 354, 0 },
    { "melony_icy_wind", 0xa348, 354, 0 },
    { "melony_icy_wind", 0x25f8, 354, 2 },
    { "melony_icy_wind", 0xa088, 354, 0 },
    { "melony_icy_wind", 0x2648, 354, 2 },
    { "melony_icy_wind", 0xa560, 354, 0 },
    { "melony_icy_wind", 0x9980, 354, 0 },
    { "melony_icy_wind", 0x9a18, 354, 0 },
    { "melony_icy_wind", 0xed38, 354, 0 },
};
    ...
    if (save::gSaveFile.randomizeGiftTMs) {
        ...
        for (auto& identifier : TM_IDENTIFIERS) {
            if (amx->cip != identifier.cip)
                continue;
            if (identifier.arg >= param_count)
                continue;
            if (params[identifier.arg + 1] != identifier.old)
                continue;
            auto rng = RngManager::NewRandomGenerator(identifier.identifier);
            params[identifier.arg + 1] = rng.RandTM();
        }
    }
    replaceGiftTMs.orig(amx, index, result, params);
});

inline void installGiftTMsHooks() {
    replaceGiftTMs.installAtPtr(pun<void*>(amx_Callback));
}